...
const InvoicePDF = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(invoiceClone, {
        scale: 1.5, // Higher quality for better resolution
        logging: false,
        useCORS: true,
        allowTaint: true,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-0">
      <div className="print:hidden flex flex-wrap justify-between items-center mb-6 gap-2">
        <div className="flex items-center">
          <Link to={`/invoices/${id}`} className="mr-4 text-gray-500 hover:text-gray-700">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Invoice PDF</h1>
        </div>
