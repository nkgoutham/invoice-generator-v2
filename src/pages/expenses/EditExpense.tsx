...
import { ArrowLeft, Upload, Calendar, DollarSign, Tag, Building, Fimport { ArrowLeft, Upload, Calendar, DollarSign, Tag, Building, FileText, Trash2, ExternalLink, Plus, FileInput as FileInvoice, X } from 'lucide-react'ount: number;
  description: string;
  category_id: string;
  client_id?: string;
  invoice_id?: string;
...
const EditExpense = () => {
  ...
  useEffect(() => {
    if (selectedExpense && !formLoaded) {
      reset({
        date: selectedExpense.date,
        amount: selectedExpense.amount,
        description: selectedExpense.description,
        category_id: selectedExpense.category_id || '',
        client_id: selectedExpense.client_id || '',
        invoice_id: selectedExpense.invoice_id || '',
        is_recurring: selectedExpense.is_recurring || false,
        recurring_frequency: selectedExpense.recurring_frequency || '',
        notes: selectedExpense.notes || '',
        is_billable: !!selectedExpense.client_id,
        is_reimbursable: selectedExpense.is_reimbursable || false,
        reimbursed: selectedExpense.reimbursed || false,
  ...
  }, [selectedExpense, reset, formLoaded]);
  ...
  const onSubmit = async (data: ExpenseFormData) => {
    ...
    try {
      ...
      if (receiptFile) {
        const newReceiptUrl = await uploadReceipt(receiptFile, user.id);
        if (newReceiptUrl) {
          receiptUrl = newReceiptUrl;
        }
      } else if (receiptPreview === null && selectedExpense?.receipt_url) {
        // If receipt preview is cleared but there was a previous receipt, set to null
        receiptUrl = null;
      }
