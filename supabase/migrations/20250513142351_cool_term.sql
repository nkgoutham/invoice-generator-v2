/*
  # Employee Salary Tracking Migration

  1. New Tables
    - `employees` table for storing employee information
      - Basic information (name, email, phone, etc.)
      - Compensation details (hourly_rate, monthly_salary)
      - Status tracking (join_date, status)

  2. Schema Updates
    - Adding salary tracking fields to `expenses` table
      - employee_id - Reference to employees
      - is_salary - Flag for salary payments
      - salary_type - Type of salary (hourly, monthly, project)
      - hours_worked - For hourly salary payments
